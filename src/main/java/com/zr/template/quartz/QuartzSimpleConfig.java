...
public class QuartzSimpleConfig {
    ...
    //Scheduler：代表独立于Quartz 的运行容器。
    // 在Scheduler 中注册了Trigger和JobDetail。
    // 它们在调度程序中具有自己的名称（名称）和组名称（Group）。
    // 触发器和JobDetail名称和组名称的组合必须唯一，但是触发器名称和组名称的组合可以与JobDetail相同。
    // 一个Job可以绑定到多个触发器，也可以不绑定。
